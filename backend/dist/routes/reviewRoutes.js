...
router.post('/course/:id', authMiddleware_1.authenticate, async (req, res) => {
    res.status(501).json({ message: 'Not implemented yet' });
});
// Delete a course review
router.delete('/course/:id', authMiddleware_1.authenticate, async (req, res) => {
    res.status(501).json({ message: 'Not implemented yet' });
});
...
router.get('/tutoring/:id', async (req, res) => {
    try {
        ...
        const reviews = await index_1.prisma.tutoringReview.findMany({
            where: { sessionId: id },
            include: {
                student: {
                    select: {
...
router.post('/tutoring/:id', authMiddleware_1.authenticate, async (req, res) => {
    try {
        if (!req.user) {
        ...
        const existingReview = await index_1.prisma.tutoringReview.findUnique({
            where: {
                studentId_sessionId: {
        ...
        // Create the review
        const review = await index_1.prisma.tutoringReview.create({
            data: {
                sessionId: id,
                studentId: req.user.id,
                rating,
...
router.delete('/tutoring/:id', authMiddleware_1.authenticate, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ message: 'Not authenticated' });
        }
        const { id } = req.params;
        // Find the review
        const review = await index_1.prisma.tutoringReview.findFirst({
            where: {
                sessionId: id,
                studentId: req.user.id
            }
        ...
        // Delete the review
        await index_1.prisma.tutoringReview.delete({
            where: { id: review.id }
        });
