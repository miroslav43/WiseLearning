...
import { deleteCourseWithCascade, updateCourseTopics } from '../services/courseService';
...
export const createCourse = async (req: Request, res: Response) => {
  try {
    ...
    if (topics.length > 0) {
      await updateCourseTopics(course.id, topics);
    }
...
export const updateCourse = async (req: Request, res: Response) => {
  try {
    ...
    if (topics) {
      await updateCourseTopics(id, topics);
    }
...
export const deleteCourse = async (req: Request, res: Response) => {
  try {
    ...
    await deleteCourseWithCascade(id);
