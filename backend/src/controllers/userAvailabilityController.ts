...
export const addAvailabilitySlot = async (req: Request, res: Response) => {
  try {
    if (!req.user) {
      return res.status(401).json({ message: 'Not authenticated' });
    }
    ...
    const existingSlots = await prisma.userAvailability.findMany({
      where: {
        userId: req.user.id,
        dayOfWeek,
        OR: [
    ...
    const newSlot = await prisma.userAvailability.create({
      data: {
        userId: req.user.id,
        dayOfWeek,
...
export const removeAvailabilitySlot = async (req: Request, res: Response) => {
  try {
    ...
    const slot = await prisma.userAvailability.findUnique({
      where: { id: slotId },
      select: { userId: true }
    });
...
export const updateAvailabilitySlot = async (req: Request, res: Response) => {
  try {
    ...
    const updatedSlot = await prisma.userAvailability.update({
      where: { id: slotId },
      data: updateData
    });
