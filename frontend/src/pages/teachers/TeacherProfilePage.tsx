...
const TeacherProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTeacher = () => {
      ...
      // For now, we'll use the first session's teacher data as a placeholder
      const allSessions = tutoringService.getApprovedTutoringSessions();
      const teacherSessions = allSessions.filter(session => session.teacherId === teacherId);
      
      if (teacherSessions.length > 0) {
        const session = teacherSessions[0];
        
        // Create a mock teacher object based on the session
        setTeacher({
  ...
  }, [teacherId, tutoringService, user, refreshKey]);
    ...
    return (
      <div className="container mx-auto px-4 py-12">
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground mb-6">Profesorul căutat nu a fost găsit sau nu mai este activ.</p>
          <Button asChild>
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      <div className="mb-6">
        <Link to="/tutoring" className="text-brand-600 hover:text-brand-700 flex items-center gap-1">
        ...
        <div className="lg:col-span-2">
          <Tabs defaultValue="about" className="w-full">
            ...
            <TabsContent value="sessions" className="space-y-6">
              <h2 className="text-xl font-semibold mb-4">Sesiuni disponibile</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {sessions.map(session => (
                    <TutoringSessionCard 
                      ...
                      session={session}
                      showDetails={true}
                      onSelect={() => window.location.href = `/tutoring?session=${session.id}`}
                    />
                  ))}
                </div>
              ) : (
